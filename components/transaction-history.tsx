...
export function TransactionHistory({ apiKey }: TransactionHistoryProps) {
  ...
  const fetchTrades = async () => {
    ...
    try {
      ...
      // Buscar apenas as últimas 10 operações
      ...
      if (response.ok) {
        ...
        console.log("[HISTÓRICO] Trades da API para histórico (últimas 10):", data)
        ...
        // Limitar a 10 operações mais recentes
        setTransactions(apiTransactions.slice(0, 10))
      } else {
  ...
  useEffect(() => {
    // Buscar as últimas 10 operações ao carregar o componente
    console.log('[HISTÓRICO] Carregando últimas 10 operações...')
    setLoading(true)
    fetchTrades()
  }, [apiKey])
          ...
          <div className="space-y-4">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="animate-pulse">
                <div className="flex items-center justify-between p-4 rounded-xl" style={{ backgroundColor: 'rgba(25, 26, 22, 0.3)' }}>
                  <div className="w-10 h-10 rounded-full" style={{ backgroundColor: 'rgba(25, 26, 22, 0.6)' }}></div>
                  <div>
                    <div className="h-4 w-24 rounded mb-2" style={{ backgroundColor: 'rgba(25, 26, 22, 0.6)' }}></div>
                    <div className="h-3 w-32 rounded" style={{ backgroundColor: 'rgba(25, 26, 22, 0.6)' }}></div>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="space-y-4">
          {transactions.length === 0 ? (
            ...
            transactions.map((transaction) => (
              <div
                key={transaction.id}
                className="flex items-center justify-between p-4 rounded-xl transition-all duration-300 hover:scale-[1.02] border border-border/30"
                style={{ backgroundColor: 'rgba(25, 26, 22, 0.3)' }}
                onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'rgba(25, 26, 22, 0.4)'}
                onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'rgba(25, 26, 22, 0.3)'}
              >
                <div className="flex items-center gap-4">
                  <div className="flex items-center justify-center w-10 h-10 rounded-full" style={{ backgroundColor: 'rgba(25, 26, 22, 0.4)' }}>
