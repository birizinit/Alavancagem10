...
export default function TradingScreen({ onBack, apiKey }: TradingScreenProps) {
  ...
  useEffect(() => {
    ...
    // Polling mais frequente quando há operações pendentes
    const tradesInterval = setInterval(() => {
      if (pendingTrades.length > 0 || botActive) {
        console.log("[v0] Trading Screen - Atualizando trades pendentes:", pendingTrades.length)
        fetchTrades()
        fetchWallets()
      }
    }, 5000)
  ...
  }, [pendingTrades.length, botActive])
  ...
  const openBotTrade = async (direction: "BUY" | "SELL") => {
    try {
      ...
      const requestBody = {
        isDemo: isDemoMode,
        amount: 1,
        closeType: "00:01" as const, // 1 minuto - menor tempo válido disponível
